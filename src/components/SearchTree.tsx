...
const TreeNode: Component<{ node: SearchResult; onSelect: (id: string) => void }> = (props) => {
    ...
    return (
        <div class="animate-in">
            <div
                class="flex items-center justify-between p-4 rounded-xl glass hover:bg-foreground/5 cursor-pointer transition-all border border-transparent hover:border-border group shadow-sm shadow-black/5"
                ...
                <div class="flex items-center gap-3">
                    <Show when={props.node.children.length > 0}>
                        <button
                            ...
                            class={`p-1 rounded-md hover:bg-foreground/10 text-muted-foreground transition-all duration-200 ${expanded() ? 'rotate-90 text-foreground' : ''}`}
                        >
                    ...
                    <div>
                        <div class="font-bold text-foreground flex items-center gap-3 font-outfit">
                            {props.node.name}
                            <span class={`text-[9px] px-2 py-0.5 rounded-full border border-border text-muted-foreground uppercase tracking-widest font-mono ${props.node.type === 'bundle' ? 'bg-indigo-500/10 text-indigo-500 dark:text-indigo-300' : ''}`}>
                                {props.node.type}
                            </span>
                        </div>
            ...
            <Show when={expanded() && props.node.children.length > 0}>
                <div class="ml-8 mt-2 flex flex-col gap-2 border-l-2 border-border pl-4 py-2">
