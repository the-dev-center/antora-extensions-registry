---
name: "your-extension-id"
description: "A short, catchy description for the registry card."
screenshots:
  - url: "https://raw.githubusercontent.com/user/repo/main/docs/assets/screenshot1.png"
    caption: "Desktop View - Dark Mode"
  - url: "https://raw.githubusercontent.com/user/repo/main/docs/assets/screenshot2.png"
    caption: "Mobile Responsive Layout"
---

= Overview

Add a detailed description of your extension here. You can use *AsciiDoc* to format your content. 
This content will be displayed in the "Insights" panel of the Antora Extensions Registry.

== Key Features

* Feature 1
* Feature 2
* Feature 3
